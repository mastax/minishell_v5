...
/*   Updated: 2024/09/09 18:33:05 by elel-bah         ###   ########.fr       */
/*                                                                            */
/******************************************************************************/

#include "../mini_shell.h"

void init_setup_context(t_setup_context *setup_ctx, t_command_context *context, 
...
void init_command_context1(t_command_context1 *ctx, t_command_context *context, 
                                  t_execution_data *exec_data)
{
    ctx->cmd = context->cmd;
    ctx->env = context->env;
...
void configure_child_io(t_command_context1 *ctx, t_arg *current_cmd, int cmd_index)
{
    int i;
...
// static void configure_child_io(t_command_context1 *ctx, t_arg *current_cmd, int cmd_index)//last
// {
//     if (cmd_index > 0)
//         dup2(ctx->pipe_fds[cmd_index - 1][0], STDIN_FILENO);
//     if (cmd_index < ctx->pipe_count)
...
void execute_child_process(t_command_context1 *ctx, t_arg *current_cmd)
{
    char *cmd_path;
    int builtin_result;
    ...
    {
        builtin_result = execute_builtin_ch(current_cmd, ctx->env, ctx->exit_status);
        exit(builtin_result);
    }
...
void handle_parent_builtin(t_command_context1 *ctx, t_arg *current_cmd)
{
    if (apply_redirections(current_cmd->red) == -1)
    {
        restore_io(ctx->io);
